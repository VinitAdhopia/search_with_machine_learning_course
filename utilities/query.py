...
# Classication constants
query_classification_model = "../../datasets/fasttext/queries.bin"
# If we cannot get to a cumulative score of 0.5 with 3 predictions, these predictions probably aren't that helpful
...
def create_query(user_query, click_prior_query, filters, sort="_score", sortDir="desc", size=10, source=None):
    ...
    # If the synonyms parameter has been set, then use our synonyms field instead of the regular name field
    name_field = "name"
    if args.synonyms:
...
def search(client, user_query, index="bbuy_products", sort="_score", sortDir="desc"):
    ...
    # Accumulate category predictions until we run out of categories or meet the threshold
    if len(predictions) >= 2:
        ...
        if len(categories) > 0 and len(categories) == len(scores):
            for idx, category in enumerate(categories):
                category_ids.append(category[category_prefix_len:])
                cumulative_threshold += scores[idx]
                # Only pick enough categories to get past the threshold.  Additional categories will just add noise.
                if cumulative_threshold >= classification_threshold:
                    break
    ...
    if cumulative_threshold >= classification_threshold:
        ...
        filters = [
            {
                "match" : { "categoryPathIds" : " OR ".join(category_ids)}
            }
        ]
...
if __name__ == "__main__":
    ...
    general.add_argument('--synonyms', action="store_true", default=False ,help='Whether to use synonyms as part of your query')
